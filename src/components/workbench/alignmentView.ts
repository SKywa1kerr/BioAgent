...
function padAlignment(value: string, length: number): string {
...
export function buildCoordinateMap(refGapped: string, queryGapped: string): CoordinateMap {
  ...
  const gappedToRef: Array<number | null> = [];
  const gappedToQuery: Array<number | null> = [];
  let refPosition = 0;
  let queryPosition = 0;
...
function buildMatchLine(refLine: string, queryLine: string, matches: boolean[] | undefined): string {
...
function mapReferenceRange(start: number | undefined, end: number | undefined, map: CoordinateMap): AlignmentRange | null {
...
function findInsertionGap(refLine: string, queryLine: string, anchor: number): number | undefined {
  const isInsertionColumn = (index: number): boolean => refLine[index] === "-" && queryLine[index] !== "-";
...
function mapMutationRanges(sample: WorkbenchSample, map: CoordinateMap, refLine: string, queryLine: string): MutationRange[] {
