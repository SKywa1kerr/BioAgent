...
1. **Web 应用和 AI Agent 解耦**：Web Dashboard 不包含聊天面板，Claude Code CLI 本身就是最好的对话界面
2. **MCP Server 独立于 Web 后端**：作为单独进程运行，Claude Code 直连，不经 Web 中转
3. **零 API 依赖**：核心分析流程（模式 A）完全本地运行，不需要任何外部 API
4. **可部署性**：支持 Docker 部署，其他用户可以在自己的服务器上搭建
