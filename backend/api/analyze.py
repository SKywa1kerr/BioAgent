...
from backend.db.database import get_session_factory, db_session
...
def trigger_analysis(req: AnalyzeRequest, background_tasks: BackgroundTasks):
    ...
    aid = new_id()
    with db_session() as session:
        analysis = Analysis(id=aid, name=req.name or f"Analysis {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M')}", source_type="scan", source_path=req.ab1_dir)
